...
class dropDownListForOutputDevices extends StatelessWidget {
  const dropDownListForOutputDevices({Key? key}) : super(key: key);
      ...
      child: Column(
        children: [
          MultiSelectBottomSheetField<String?>(
            items: controller.itemsOutputDevices.map((item) => MultiSelectItem<String?>(item, item)).toList(),
            initialValue: controller.selectedOutputDevices, // Bind initial values
            title: Text("Select output Devices"),
            decoration: BoxDecoration(
            ...
            onConfirm: (List<String?> values) {
              ...
              controller.updateSelectedOutputDevices(values.whereType<String>().toList());
            },
            maxChildSize: 0.5, // Controls the height of the bottom sheet (50% of screen height)
            chipDisplay: MultiSelectChipDisplay<String>(
              items: controller.selectedOutputDevices
                  .map((item) => MultiSelectItem<String>(item, item))
                  .toList(),
              chipColor: Colors.green.withOpacity(0.1),
              textStyle: TextStyle(color: Colors.green[800]),
              onTap: (value) {
                controller.removeOutputDevice(value); // Remove item using the controller
              },
            ),
          ),
        ],
